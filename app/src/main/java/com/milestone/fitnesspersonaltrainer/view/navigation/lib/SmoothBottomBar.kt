...
class SmoothBottomBar @JvmOverloads constructor(
    ...
    fun setupWithNavController(viewpager: VerticalViewpager){
        NavigationComponentHelper.setupWithNavController(viewpager, this)
    }
            ...
            for ((i, item) in items.withIndex()) {
                if (item.rect.contains(event.x, event.y)) {
                    if (i != itemActiveIndex) {
                        ...
                        println("------------------>Listener : $onItemSelectedListener")
                    } else {
